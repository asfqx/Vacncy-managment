<template>
  ...
  >
    <template #aside>
      ...
      <UiCard title="Офис" :subtitle="stats.published" :active="activeFilter === 'published'" @click="activeFilter = 'published'" />
    ...
    <template v-else>
      <section class="heroPanel panel">
        <div>
          ...
          <p v-if="actionMessage" class="actionText actionText--success">{{ actionMessage }}</p>
          <p v-if="actionError" class="actionText actionText--error">{{ actionError }}</p>
        </div>
        ...
        >
          ...
          <div class="vacancyCard__footer">
            ...
            <button
              class="dangerBtn"
              type="button"
              :disabled="deletingVacancyUuid === vacancy.uuid"
              @click.stop="deleteVacancy(vacancy)"
            >
              {{ deletingVacancyUuid === vacancy.uuid ? "Удаляем..." : "Удалить" }}
            </button>
          </div>
        </article>
      </section>
...
const emptyTitle = computed(() => {
  ...
  if (activeFilter.value === "published") return "Офисных вакансий пока нет";
...
const emptyText = computed(() => {
  ...
  if (activeFilter.value === "published") return "Когда у компании появятся офисные вакансии, вы увидите их здесь.";
...
async function deleteVacancy(vacancy) {
  const title = vacancy?.title || "эту вакансию";
  const confirmed = window.confirm(`Удалить вакансию "${title}"?`);
  if (!confirmed) return;

  deletingVacancyUuid.value = vacancy.uuid;
  actionError.value = "";
  actionMessage.value = "";

  try {
    await vacanciesApi.delete(vacancy.uuid);
    await loadVacancies();
    actionMessage.value = `Вакансия "${title}" удалена.`;
  } catch (e) {
    const status = e?.response?.status;
    if (status === 404) actionError.value = "Вакансия уже удалена или не найдена.";
    else if (status === 400) actionError.value = "Эту вакансию нельзя удалить.";
    else actionError.value = "Не удалось удалить вакансию.";
  } finally {
    deletingVacancyUuid.value = "";
  }
}
...
async function loadVacancies() {
  ...
  try {
    ...
    vacancies.value = (await vacanciesApi.getAll({ company_id: myCompany.uuid, limit })) || [];
  } catch (e) {
    const status = e?.response?.status;
    if (status === 404) {
...
.panel,
.errorPanel,
.vacancyCard { border-radius: 24px; border: 1px solid rgba(255,255,255,0.08); background: linear-gradient(180deg, rgba(18,19,27,0.96), rgba(11,12,17,0.98)); box-shadow: 0 18px 44px rgba(0,0,0,0.24); }
.heroPanel,
.emptyPanel { padding: 24px; display: flex; justify-content: space-between; gap: 18px; align-items: center; }
.eyebrow,
.vacancyCard__eyebrow { margin: 0 0 8px; color: #8eb4ff; font-size: 12px; text-transform: uppercase; letter-spacing: 0.12em; }
.heroTitle,
.emptyTitle { margin: 0; font-size: 30px; line-height: 1.1; }
.heroText { margin: 12px 0 0; max-width: 52ch; color: rgba(255,255,255,0.72); line-height: 1.65; }
.actionText { margin: 12px 0 0; line-height: 1.5; }
.actionText--success { color: #91f2b0; }
.actionText--error { color: #ff9d9d; }
.heroAction { min-height: 46px; padding: 0 18px; border-radius: 16px; display: inline-flex; align-items: center; justify-content: center; text-decoration: none; color: #fff; font-weight: 700; border: 1px solid rgba(47,115,255,0.4); background: linear-gradient(135deg, #2f73ff, #5a93ff); box-shadow: 0 10px 22px rgba(47,115,255,0.24); }
.listGrid { display: grid; grid-template-columns: repeat(2, minmax(0, 1fr)); gap: 16px; }
.vacancyCard { padding: 18px; display: grid; gap: 12px; cursor: pointer; transition: transform 0.2s ease, border-color 0.2s ease; }
...
.badge { display: inline-flex; align-items: center; min-height: 34px; padding: 0 12px; border-radius: 999px; border: 1px solid rgba(255,255,255,0.14); color: rgba(255,255,255,0.9); background: rgba(255,255,255,0.04); }
...
.vacancyCard__footer { display: flex; justify-content: space-between; gap: 12px; font-size: 13px; padding-top: 6px; border-top: 1px solid rgba(255,255,255,0.06); }
.dangerBtn { min-height: 34px; padding: 0 12px; border-radius: 12px; border: 1px solid rgba(255,113,113,0.35); background: rgba(255,113,113,0.12); color: #ffb0b0; font-weight: 700; cursor: pointer; }
.dangerBtn:disabled { opacity: 0.65; cursor: not-allowed; }
